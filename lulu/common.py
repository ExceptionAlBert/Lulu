...
def urlopen_with_retry(*args, **kwargs):
    ...
    for i in range(retry_time):
        try:
            return session.get(*args, verify=False, **kwargs)
...
def post_content(url, headers=FAKE_HEADERS, post_data={}):
    ...
    response = urlopen_with_retry(url, headers=headers, data=post_data)
    data = response.text
    return data


def url_size(url, headers=FAKE_HEADERS):
    response = urlopen_with_retry(url, stream=True, headers=headers)
...
def url_info(url, headers=FAKE_HEADERS):
...
def url_save(
    ...
    **kwargs
    ...
    if received < file_size:
        ...
        response = urlopen_with_retry(url, stream=True, **kwargs)
        try:
            range_start = int(
                response.headers[
                    'content-range'
